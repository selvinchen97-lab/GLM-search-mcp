"""Planning state machine for selvin-search-mcp.

Layered, complexity-adaptive search planning. Key features over the
naive version:
  - **Persistence**: each session is mirrored to
    ~/.config/selvin-search/sessions/{sid}.json so MCP process restarts
    don't lose context.
...
SESSION_DIR = Path.home() / ".config" / "selvin-search" / "sessions"
...
def _ensure_dir() -> Path:
    try:
        ...
        alt = Path("/tmp/selvin-search/sessions")
